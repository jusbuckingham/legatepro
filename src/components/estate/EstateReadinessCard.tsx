...
function severityClasses(sev: ReadinessSignal["severity"]): string {
  switch (sev) {
    case "high":
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-4 shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <div className="mt-1 text-sm text-slate-200" aria-live="polite">
              Loading readiness…
            </div>
          </div>
    ...
    <section className="rounded-xl border border-slate-800 bg-slate-950/70 p-4 shadow-sm">
      ...
      {(missingCount > 0 || riskCount > 0) && (
        <details className="mt-4 rounded-lg border border-slate-800 bg-slate-950/60 p-3">
          <summary
            className={cx(
              "cursor-pointer text-xs font-medium text-slate-200",
              "rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-200/20 focus-visible:ring-offset-2 focus-visible:ring-offset-slate-950",
            )}
          >
            Why this score?
