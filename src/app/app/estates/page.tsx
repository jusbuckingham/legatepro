...
export default async function EstatesPage() {
  ...
  const estates = await Estate.find({ ownerId: MOCK_OWNER_ID })
    .sort({ createdAt: -1 })
    .lean();
        ...
        <ul className="divide-y divide-slate-800 rounded-lg border border-slate-800 bg-slate-900/40">
          {estates.map((estate: any) => (
            <li
              ...
              className="flex items-center justify-between px-4 py-3 hover:bg-slate-900/70"
            >
              <div>
                <Link
                  href={`/app/estates/${estate._id.toString()}`}
                  className="font-medium text-slate-50 hover:text-emerald-400"
                >
                  {estate.label}
                </Link>
                <p className="text-xs text-slate-400">
                  Status: {estate.status || "OPEN"} • Created{" "}
                  {estate.createdAt
              ...
              <span className="rounded-full bg-slate-800 px-3 py-1 text-xs text-slate-300">
                {estate.decedent?.fullName || "Decedent not set"}
              </span>
            </li>
          ))}
        </ul>
