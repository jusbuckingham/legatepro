  ...
  return (
    <div className="mx-auto max-w-5xl space-y-8">
      ...
      <header className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
        <div>
          ...
          <p className="mt-1 max-w-2xl text-sm text-slate-400">
            Manage your LegatePro plan, see what&apos;s included, and get ready
            for Stripe-powered billing. We&apos;ll never charge you without a
            clear, explicit upgrade step.
          </p>
        </div>

        <div className="rounded-lg border border-emerald-500/40 bg-emerald-950/30 px-4 py-3 text-xs text-emerald-100 shadow-sm">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-emerald-300">
            Current plan
          </div>
          <div className="mt-1 text-sm font-medium">
            {currentPlanId === "free" ? "Starter (Free)" : "Pro Personal Representative"}
          </div>
        ...
        {PLANS.map((plan) => {
          ...
          return (
            <div
              key={plan.id}
              className={[
                "relative flex h-full flex-col rounded-xl border bg-slate-950/40 p-5 shadow-sm",
                isPro
            ...
            >
              ...
              <div className="mt-auto">
                {isCurrent ? (
                  <button
                    type="button"
                    className="inline-flex w-full items-center justify-center rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-xs font-medium text-slate-200 shadow-sm"
                    disabled
                  >
                    Current plan
                  </button>
                  ...
                  <button
                    type="button"
                    className="inline-flex w-full items-center justify-center rounded-md border border-[#F15A43]/80 bg-[#F15A43] px-3 py-2 text-xs font-semibold text-slate-950 shadow-sm hover:bg-[#f26b56]"
                  >
      ...
      <section className="rounded-xl border border-slate-800 bg-slate-950/50 px-4 py-4 text-xs text-slate-300">
        ...
        <p className="mb-2">
          LegatePro will use Stripe for secure payments and subscription
          management. While billing is in beta, you&apos;ll be able to use the
